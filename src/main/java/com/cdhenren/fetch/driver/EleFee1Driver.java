 ...
 * @date 2018/04/28
 */
public class EleFee1Driver extends AbstractRealEstate {
    ...
    public EleFee1Driver(TaskWrap taskWrap) {
        super(taskWrap);
    }

    public EleFee1Driver(TaskWrap taskWrap, Semaphore semaphore) {
    ...
    protected List<ResultSet> parse() {
        ...
        List<ResultSet> tmpEleFee1s = new ArrayList<>(lines.size());
        for (Element line : lines) {
            TmpEleFee1 tmpEleFee1 = new TmpEleFee1();
            ...
            reflectEntity(tmpEleFee1, values);
            if (StringUtils.isEmpty(tmpEleFee1.get省())) {
                continue;
            }
            tmpEleFee1s.add(tmpEleFee1);
        }
        return tmpEleFee1s;
    }
